...
@RestController
@RequestMapping("service/execution/status/details")
@CrossOrigin(origins = "*")
public class ExecutionStatusDetailsController {
  private final ExecutionStatusDetailsService executionStatusDetailsService;

  @Autowired
  public ExecutionStatusDetailsController(
  ...
  Page<ExecutionStatusDetails> getPage(
      ...
      int page,
      int size) {
    return executionStatusDetailsService.getPage(
        campaignId,
        deliveryId,
